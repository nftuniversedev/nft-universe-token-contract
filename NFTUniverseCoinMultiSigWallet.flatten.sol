...
// File: contracts/NFTUniverseCoin.sol
 ...
 * @title coin contract
 */
contract NFTUniverseCoin is ERC20Capped, ERC20Detailed, ERC20Burnable {
...
// File: contracts/NFTUniverseCoinSafeMath.sol
 ...
 */
library NFTUniverseCoinSafeMath {
...
// File: contracts/NFTUniverseCoinMultiSigWallet.sol
...
/**
 * @title NFTUniverseCoin Multi Signature contract
 */
contract NFTUniverseCoinMultiSigWallet {
    /*  Enums  */
    ...
    /* Contract Created */
    event MultiSigCreated(address indexed creator, address indexed NFTUniverseCoin);
    ...
    using NFTUniverseCoinSafeMath for uint8;
    using SafeMath for uint256;
    ...
    NFTUniverseCoin public NFTUniverseCoin;
     ...
     * @param _NFTUniverseCoinAddress NFTUniverseCoin address
     *
     */
    constructor (address[] memory _signers, uint8 _requiredSign, address _NFTUniverseCoinAddress, string memory _description) public {
        ...
        nextTx = 0;
        NFTUniverseCoin = NFTUniverseCoin(_NFTUniverseCoinAddress);
        owner = msg.sender;
        description = _description;
        emit MultiSigCreated(owner, _NFTUniverseCoinAddress);
    }
    ...
    /**
      ...
      * @param _amountUnit amount of NFTUniverseCoin(Play part) to be transferred
      * @param _amountMinunit amount of NFTUniverseCoin(Minunit part) to be transferred
      */
    ...
    // NFTUniverseCoin.transfer will be fail with reason out of gas. Then the TX will remain
    ...
    function runTX(uint16 txid) internal senderIsSigner returns(bool) {
        ...
        if(request.txType == TxType.TRANSFER) {
            // transfer money
            NFTUniverseCoin.transfer(request.who, request.amount);
            ...
            NFTUniverseCoin.setAdmin(request.who);
        } else if(request.txType == TxType.OWNER_CHANGE) {
            NFTUniverseCoin.setOwner(request.who);
        } else if(request.txType == TxType.VAULT_CHANGE) {
            NFTUniverseCoin.setVault(request.who);
        } else {
            return false;
        }
        return true;
