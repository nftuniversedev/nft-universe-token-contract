...
/**
 * NFTUniverseCoin contract learning test
 *   deploy in before, and no deploy between it()
 */
contract("NFTUniverseCoin", (accounts) => {
  ...
  let NoOfTokensInMinunit, OneNFTUniverseCoinInMinunit, NoOfTokensInNFTUniverse;
  let coin
  ...
  before(async () => {
    ...
    OneNFTUniverseCoinInMinunit = BIG(await coin.getOneNFTUniverseCoin());
    NoOfTokensInNFTUniverse = NoOfTokensInMinunit.div(OneNFTUniverseCoinInMinunit)
  });
  ...
  it("coin initial values", async () => {
    ...
    assert.equal(await coin.symbol(), "NFTUniverse");
    assert.equal(await coin.name(), "NFTUniverseCoin");
    assert.equal(await coin.decimals(), 9);
    assert.equal((await coin.totalSupply()).toString(), BIG(NOOFTOKENS).mul(OneNFTUniverseCoinInMinunit).toString());
  ...
  it("token distribution", async () => {
    ...
    await coin.transfer(admin, OneNFTUniverseCoinInMinunit, {from: vault});
    assert.equal(await balanceOf(admin), OneNFTUniverseCoinInMinunit.toString());

    // transfer 0.5 token from admin to owner
    let halfNFTUniverse = OneNFTUniverseCoinInMinunit.div(BIG(2))
    await coin.transfer(owner, halfNFTUniverse, {from: admin});
    assert.equal(await balanceOf(owner), halfNFTUniverse.toString());
    assert.equal(await balanceOf(admin), halfNFTUniverse.toString());
    assert.equal(await balanceOf(vault), (NoOfTokensInNFTUniverse.sub(BIG(1)).mul(OneNFTUniverseCoinInMinunit).toString()));
  });
  ...
  it("marginal transfers", async () => {
    ...
    try {
      await coin.transfer(user2, OneNFTUniverseCoinInMinunit.mul(BIG(1)), {from: user1});
      assert.fail();
    }
    ...
    try
    {
      await coin.transfer(user2, OneNFTUniverseCoinInMinunit.mul(BIG(0)), {from: user1});
      assert.equal(await balanceOf(user1), OneNFTUniverseCoinInMinunit.mul(BIG(0)).toString());
      assert.equal(await balanceOf(user2), OneNFTUniverseCoinInMinunit.mul(BIG(0)).toString());
    }
    catch(exception) { assert.fail(); }

    // transfer 0 token from vault(balance > 0) to user1
    try {
        await coin.transfer(user1, OneNFTUniverseCoinInMinunit.mul(BIG(0)), {from: vault});
        assert.equal(await balanceOf(user1), OneNFTUniverseCoinInMinunit.mul(BIG(0)).toString());
    } catch(exception) { assert.fail(); }

    // transfer whole balance to other
    await coin.transfer(user1, OneNFTUniverseCoinInMinunit.mul(BIG(1)), {from: vault});
    assert.equal(await balanceOf(user1), OneNFTUniverseCoinInMinunit.mul(BIG(1)).toString());
    await coin.transfer(user2, OneNFTUniverseCoinInMinunit.mul(BIG(1)), {from: user1});
    assert.equal(await balanceOf(user1), OneNFTUniverseCoinInMinunit.mul(BIG(0)).toString());
    assert.equal(await balanceOf(user2), OneNFTUniverseCoinInMinunit.mul(BIG(1)).toString());
  });
  ...
  it("only admin can set reserve", async () => {
    await coin.setReserve(user1, OneNFTUniverseCoinInMinunit, {from: admin});
  ...
  it("reserve and reserveOf", async () => {
    ...
    await coin.transfer(user1, OneNFTUniverseCoinInMinunit.mul(BIG(10)), { from: vault });
    await coin.setReserve(user1, OneNFTUniverseCoinInMinunit.mul(BIG(5)), { from: admin });
    assert.equal(await reserveOf(user1), OneNFTUniverseCoinInMinunit.mul(BIG(5)).toString());
  });
  
  it("transfer after setting reservation", async () => {
    // user1 = 10 NFTUniverse
    await coin.transfer(user1, OneNFTUniverseCoinInMinunit.mul(BIG(10)), { from: vault });
    // user1 = 10 NFTUniverse with 5 NFTUniverse reserve
    await coin.setReserve(user1, OneNFTUniverseCoinInMinunit.mul(BIG(5)), { from: admin });
    // user1 = 9 NFTUniverse with 5 NFTUniverse reserve
    await coin.transfer(user2, OneNFTUniverseCoinInMinunit.mul(BIG(1)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneNFTUniverseCoinInMinunit.mul(BIG(9)).toString());

    // user1 = 6 NFTUniverse with 5 NFTUniverse reserve
    coin.transfer(user2, OneNFTUniverseCoinInMinunit.mul(BIG(3)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneNFTUniverseCoinInMinunit.mul(BIG(6)));

    try {
      let tx = await coin.transfer(user2, OneNFTUniverseCoinInMinunit.mul(BIG(2)), {from: user1}); // violating reserved amount
      assert.equal(await balanceOf(user1), OneNFTUniverseCoinInMinunit.mul(BIG(6)));
      assert.fail();
    }
    catch(exception) {
