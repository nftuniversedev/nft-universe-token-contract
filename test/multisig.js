...
const NFTUniverseCoin = artifacts.require('NFTUniverseCoin.sol');
const NFTUniverseCoinMultiSigFactory = artifacts.require('NFTUniverseCoinMultiSigWallet.sol');
const web3 = NFTUniverseCoin.web3;
const eth = web3.eth;
...
const deployMultiSigWallet = (owners, requiredConfirm, coin) => {
  return NFTUniverseCoinMultiSigFactory.new(owners, requiredConfirm, coin, "vault");
};

const deployNFTUniverseCoin = (owner,admin,vault) => {
  return NFTUniverseCoin.new("NFTUniverse", "NFTUniverseCoin", owner, admin, vault);
};
...
contract("NFTUniverseCoinMultiSigWallet", (accounts) => {
  const BIG = (v) => new theBN.BN(v)
  let coinInstance;
  let walletInstance;
  const requiredConfirmations = 2;

  let OneNFTUniverseCoinInMinunit;
  ...
  beforeEach(async () => {
    coinInstance = await deployNFTUniverseCoin(owner, admin, vault);
    assert.ok(coinInstance);

    NoOfTokensInMinunit = await coinInstance.totalSupply();
    OneNFTUniverseCoinInMinunit = await coinInstance.getOneNFTUniverseCoin();
  ...
  it("multisig wallet basic functions", async () => {
    assert.equal( await walletInstance.nFTUniverseCoin(), coinInstance.address );
  ...
  it("only signers can request TX", async () => {
    // user1 cannot
    assertRevert("1-1",() => walletInstance.requestTransferInNFTUniverse(user1, 1, 0, "test 1", {from: user1}));
    ...
    // owner cannot
    assertRevert("2-1",() => walletInstance.requestTransferInNFTUniverse(user1, 1, 0, "test 2-1", {from: owner}));
    ...
    // admin(as signer) can
    assertNotRevert("3-1",() => walletInstance.requestTransferInNFTUniverse(user1, 1, 0, "test 3-1", {from: admin}));
    ...
    // vault(as signer) can
    assertNotRevert("4-1",() => walletInstance.requestTransferInNFTUniverse(user1, 1, 0, "test 4-1", {from: vault}));
    ...
    assertNotRevert("5-1",() => walletInstance.requestTransferInNFTUniverse(user1, 1, 0, "test 5-1", {from: signer}));
  ...
  it("signers only methods", async () => {
    // request a TX
    walletInstance.requestTransferInNFTUniverse(user1, 1, 0, "test 1", {from: signer});
  ...
  it("test transfer using multisig wallet", async () => {
    ...
    const thousandNFTUniverseInMinunit = OneNFTUniverseCoinInMinunit.mul(BIG(1000))
    coinInstance.transfer(walletInstance.address, thousandNFTUniverseInMinunit, {from:vault});
    assert.equal(await balanceOf(walletInstance.address), thousandNFTUniverseInMinunit.toString());

    // Add nFTUniversecoin transfer transaction
    // example ABI generation:
    // const transferData = coinInstance.contract.transfer.getData(user1, OneNFTUniverseCoin * 1);
    //

    const MSG1 = "send 1NFTUniverse and 1DENNIS to user1";
    const trSubmit = await walletInstance.requestTransferInNFTUniverse(user1, 1, 1, MSG1, {from:vault});
    ...
    assert.equal(afterBalance.toString(), OneNFTUniverseCoinInMinunit.mul(BIG(1)).add(BIG(1)).toString());
  ...
  it("Add a signer", async () => {
    ...
    assertNotRevert("signer1",() => walletInstance.requestTransferInNFTUniverse(user2, 1, 0, "test addsigner-1", {from: user1}));
  ...
  it("Remove a signer", async () => {
    ...
    assertRevert("signer1",() => walletInstance.requestTransferInNFTUniverse(user1, 1, 0, "test addsigner-1", {from: user1}));
