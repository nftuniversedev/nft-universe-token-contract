...
const NFTUniverseCoinMultiSigFactory = artifacts.require('NFTUniverseCoinMultiSigWalletWithMint.sol');
...
contract("NFTUniverseCoinMultiSigWalletWithMint", (accounts) => {
  ...
  it("Add a signer", async () => {
    ...
    assertNotRevert("signer1",() => walletInstance.requestMintInNFTUniverse(user1, 1, 0, "test addsigner-1", {from: user1}));
  ...
  it("Remove a signer", async () => {
    ...
    assertRevert("signer1",() => walletInstance.requestMintInNFTUniverse(user1, 1, 0, "test addsigner-1", {from: user1}));
  ...
  it("Add and remove a minter", async () => {
      ...
      assertNotRevert("user1 can mint", () => coinInstance.mint(user1, OneNFTUniverseCoinInMinunit, {from: user1}));
      let afterBalance = await coinInstance.balanceOf(user1);

      // balance difference should be same as the minted amount
      assert.equal(afterBalance.sub(prevBalance).toString(), OneNFTUniverseCoinInMinunit.toString());
      ...
      assertRevert("user1 cannot mint after renounce", () => coinInstance.mint(user1, OneNFTUniverseCoinInMinunit, {from: user1}));
  ...
  it("test mint using multisig wallet", async () => {
        ...
        const thousandNFTUniverse = BIG(1000)
        const zero = BIG(0)

        const MSG1 = "MINT 1000 NFTUniverse to user1";
        const trSubmit = await walletInstance.requestMintInNFTUniverse(user1, thousandNFTUniverse, zero, MSG1, {from:vault});
        ...
        assert.equal(afterBalance.toString(), thousandNFTUniverseInMinunit.toString());
